Test scripts for Apple's CoreCrypto.

CommonCrypto
============

Apple assigned CVE-2024-23218 for this issue.

Usage
-----
Run `step0.sh`, `step1.sh` as normal. Instead of running `step2.sh` run the
`step2-alt.sh` script.

Compile this reproducer:
```
clang -O3 -o time_decrypt time_decrypt.m -framework Foundation -framework Security
```
...
./time_decrypt -i rsa2048_repeat/ciphers.bin \
    -o rsa2048_repeat/raw_times.bin -k rsa2048/key.key -n 256
```
...
-o rsa2048_repeat/ \
--binary 8 --endian little --clock-frequency 1000.0
```
The `--clock-frequency` is always 1000.0 MHz, since the measurements are done
in nanoseconds.
