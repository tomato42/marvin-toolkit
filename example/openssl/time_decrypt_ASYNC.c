...
#else
// return process CPU time to better handle time spent waiting for select()
uint64_t get_time_before() {
    struct timespec cpu_time = {0};
    int ret;

    ret = clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &cpu_time);
    if (ret != 0) {
        fprintf(stderr, "clock_gettime failure\n");
        abort();
    }

    return (uint64_t)((uint64_t)cpu_time.tv_sec * 1000000000L +
        (uint64_t)cpu_time.tv_nsec);
}

uint64_t get_time_after() {
    return get_time_before();
}
#endif
...
int decryptjob(void *arg) {

    void **args = (void **)arg;
    unsigned char *ciphertext = args[0];
    EVP_PKEY_CTX *ctx = args[1];
    unsigned char *plaintext = args[2];

    size_t plaintext_len;
    int ret = 0; // OpenSSL return convention; 0 = FAIL
    int r_ret;
    ...
    r_ret =
    EVP_PKEY_decrypt(ctx, plaintext, &plaintext_len,
                     ciphertext, ciphertext_len);

    ERR_pop_to_mark();

    if (r_ret != 1 || plaintext_len != 48) {
        fprintf(stderr, "Incorrect decryption length\n");
    } else {
        ret = 1; // OpenSSL return convention; 1 = OK
    }
...
int main(int argc, char *argv[]) {
    ...
    if (!async_ctx) {
        fprintf(stderr, "Can't start ASYNC CTX\n");
        goto err;
    }
    ...
    while ((r_ret = read(in_fd, ciphertext, ciphertext_len)) > 0) {
        ...
        job_params[0] = ciphertext;
        job_params[1] = ctx;
        job_params[2] = plaintext;

        time_before = get_time_before();

        for (;;) {
            switch (ASYNC_start_job(&job, async_ctx, &r_ret, decryptjob,
                                    job_params, sizeof(job_params))) {
                case ASYNC_ERR:
                    /* fall through */
                case ASYNC_NO_JOBS:
                ...
                case ASYNC_FINISH:
                    if (r_ret != 1) {
                        fprintf(stderr, "Job task failure\n");
                        abort();
                    }
                    goto job_end;
            }
            ...
            if (!ASYNC_WAIT_CTX_get_all_fds(async_ctx, NULL, &numfds)
                    || numfds > 1) {
                fprintf(stderr, "Unexpected number of fds\n");
                abort();
            }
            ASYNC_WAIT_CTX_get_all_fds(async_ctx, &waitfd, &numfds);
            if (numfds > 1) {
                fprintf(stderr, "Inconsistent returns\n");
                abort();
            }
            FD_ZERO(&waitfdset);
